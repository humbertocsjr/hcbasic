class Para : No
{
    ...
    protected override void CompilaInterno(Ambiente amb)
    {
        ...
        Atribuicao atrib = new Atribuicao(Trecho, new List<string>(new string[]{Variavel.Nome}));
        atrib.Local = true;
        atrib.Valor = ValorInicial;
        atrib.Inicializa(amb);
        atrib = (Atribuicao)atrib.Otimiza(amb);
        atrib.Compila(amb);
        amb.Saida.EmiteRotulo(repete);
        amb.Tipo = TipoVariavel.UInt16;
        OpLogica op =new OpLogica(Trecho, new LeiaVariavel(Trecho, LeiaVariavel.TipoLeitura.Comum, Variavel.Modulo.Nome, Variavel.Nome){Local = true}, ValorFinal);
        op.Operacao = "<=";
        op.Inicializa(amb);
        ...
        atrib = new Atribuicao(Trecho, new List<string>(new string[]{Variavel.Nome}));
        atrib.Local = true;
